...
Section rfn.
  ...
  Definition lbl_lift_r : (label -d> label -d> siProp) -d> label -d> expr -d> siProp :=
    λ L ℓ e', ((∃ t' ℓ', ⌜ rtc step_not_error e' t' ⌝ ∗ ⌜ faulty t' ℓ' ⌝ ∗ L ℓ ℓ')%I).
  ...
  Lemma rfn_impl e1 e2 Φ Φ' L L' :
    ⊢ rfn Φ' L' e1 e2 -∗ (∀ ℓ ℓ', L' ℓ ℓ' -∗ L ℓ ℓ') -∗ (∀ v v', Φ' v v' -∗ Φ v v') -∗ rfn Φ L e1 e2.
  Proof.
    iIntros "He Hl Hv".
    rewrite /rfn. iApply (wp_impl with "He [Hl]").
    - iIntros (ℓ) "H". iDestruct "H" as (t' ℓ') "(Hs & Hf & HL)". iExists t', ℓ'. iFrame. by iApply "Hl".
  ...
  Lemma rfn_faulty e ℓ (H : faulty e ℓ) e' ℓ' (H' : faulty e' ℓ') Φ L : L ℓ ℓ' ⊢ rfn Φ L e e'.
  Proof. iIntros "HΦ". rewrite /rfn /lbl_lift_r. iApply wp_faulty; eauto. Qed.
  ...
  Proof.
    ...
    - iIntros (v) "H". iDestruct "H" as (e' ℓ') "(%Hs & Hf & HL)".
      rewrite /lbl_lift_r. iExists e', ℓ'. iFrame. iPureIntro. by eapply rtc_l.
  Qed.
  ...
  Proof.
    ...
    - iIntros (v) "H". rewrite /lbl_lift_r. iDestruct "H" as (e' ℓ') "(%Hs & %Hf & HL)".
      rewrite /lbl_lift_r.
      { inversion Hs; simplify_eq.
        - exfalso. by eapply faulty_not_stop.
        - rewrite (ne_step_det _ _ _ H H0). iExists e', ℓ'. iSplit; auto. }
  Qed.
